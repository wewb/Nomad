...
class StatisticsService {
    // 基础统计指标
    static async getBasicMetrics(projectId, startDate, endDate) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
        const result = await Event_1.Event.aggregate([
            { $match: match },
            {
                $group: {
                    _id: null,
    ...
    // 用户环境分析
    static async getEnvironmentMetrics(projectId, startDate, endDate) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
    ...
    // 用户行为分析
    static async getBehaviorMetrics(projectId, startDate, endDate) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
    ...
    // 时间维度分析
    static async getTimeMetrics(projectId, startDate, endDate, interval = 'day') {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
        ...
        const trends = await Event_1.Event.aggregate([
            { $match: match },
            {
                $group: {
        ...
        const [currentPeriod, previousPeriod] = await Promise.all([
            Event_1.Event.countDocuments({ ...match }),
    ...
    // 用户路径分析
    static async getPathAnalysis(projectId, startDate, endDate) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
    ...
    // 转化漏斗分析
    static async getFunnelAnalysis(projectId, startDate, endDate, steps) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
        ...
        const stepsData = await Promise.all(steps.map(async (step, index) => {
            ...
            let users = await Event_1.Event.aggregate([
                { $match: match },
                { $unwind: '$data.events' },
                { $match: eventMatch },
                {
            ...
            if (index > 0) {
                ...
                const prevUsers = await Event_1.Event.aggregate([
                    { $match: match },
                    { $unwind: '$data.events' },
    ...
    // 获取事件分析数据
    static async getEventAnalysis(projectId, startDate, endDate) {
        const match = {
            projectId,
            createdAt: { $gte: startDate, $lte: endDate }
        };
        const [eventStats, userBehavior, sourceAnalysis] = await Promise.all([
            // 事件基础统计
            Event_1.Event.aggregate([
                { $match: match },
                { $unwind: '$data.events' },
            ...
            Event_1.Event.aggregate([
                { $match: match },
                {
                    $group: {
                        _id: '$userEnvInfo.uid',
                        sessionCount: { $sum: 1 },
                        avgSessionDuration: { $avg: '$data.duration' },
                        totalPageViews: {
                            $sum: {
                                $size: {
                                    $filter: {
        ...
        return {
            ...
            userBehavior: {
                ...
                avgSessionsPerUser: userBehavior.reduce((acc, cur) => acc + cur.sessionCount, 0) / userBehavior.length,
                avgSessionDuration: userBehavior.reduce((acc, cur) => acc + (cur.avgSessionDuration || 0), 0) / userBehavior.length,
                avgPageViewsPerSession: userBehavior.reduce((acc, cur) => acc + cur.totalPageViews, 0) / userBehavior.length
            },
            sourceAnalysis
        };
    }
